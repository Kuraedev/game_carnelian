...
@onready var stamina_bar: ProgressBar = $TopLeft/Stats/StaminaBar
...
func _bind_player_health() -> void:
	...
	if player.has_signal("stamina_changed"):
		...
		_on_stamina_changed(player.stamina, player.stats.max_stamina)

func _on_health_changed(cur: float, mx: float) -> void:
	health_bar.max_value = mx
	health_bar.value = cur

func _on_stamina_changed(cur: float, mx: float) -> void:
